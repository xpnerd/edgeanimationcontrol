...
ListControl.prototype.find = ListControl_find;
...
function ListControl_pick(theLabel, optionalMatchFn, optionalSetOptionSelected) {
    ...
    else if ((this.object.editable) && (this.object.editable == "true")) { //add functionality to work with editable dropdowns
...
function ListControl_pickValue(theValue, optionalMatchFn, optionalSetOptionSelected) {
    ...
    with (this) {
        ...
        else if ((object.editable) && (object.editable == "true")) { //add functionality to work with editable dropdowns
...
//   If an index is specified, returns the display value of that index.
...
function ListControl_get(optionalIndex) {
    ...
    with (this) {
        ...
        else if (optionalIndex == null && index == -1) { //for editable dropdowns...
...
function ListControl_setIndex(theIndex) {
    var retVal = false;
    with (this) {
        if (theIndex < list.length) {
            object.selectedIndex = theIndex;
            index = theIndex;
            //wmc - just selecting is not enough for an editable, need to set the text region.
            if ((object.editable) && (object.editable == "true") && index >= 0) {
...
function ListControl_add(newItemStr, newValue) {
    ...
    with (this) {
        ...
        index = object.selectedIndex;
        if (object.editable == "true" && index < 0 || list.length == 0) {
...
function ListControl_del(optionalIndex) {
    ...
    with (this) {
        ...
        if (optionalIndex == null) // if using selection
        {
...
//
// DESCRIPTION:
//   Sets the innerHTML of the list object based on the values
...
//
// DESCRIPTION:
//   Returns true if 'itemStr' is in the list control
//			 flase else;
//
// ARGUMENTS:
//   itemStr - item to be found in the list
//
// RETURNS:
//   bool
//-------------------------------------------------------------------

function ListControl_find(itemStr) {
    var retVal = false;
